(function ($) {
    ...
    $.fn.SO1120A7 = function (method, param) {
        try {
            ...
            return this.each(function () {
                try {
                    ...
                    if (state) {
                        ...
                        $.data(this, formName, {
                            options: $.extend({}, new defaults(), new SO1120A7(), method)
                        });
                        formLoaded(this);
                    }
                }
                catch (err) {
                    errorHandle(formName, '$.fn.SO1120A7_each', err);
                }
            });
        }
        catch (err) {
            errorHandle(formName, '$.fn.SO1120A7', err);
        }
    };
    ...
    function changeMode(div, em) {
        try {
            ...
            disableChildControls(div, getControlObject(div, 'frmSO1120A7'), options.controls, flag);
        }
        catch (err) {
    ...
    function initcsListData(div) {
        try {
            var options = $.data(div, formName).options;
            //csDepositPTCode, csBPCode
            getControlObject(div, 'csDepositPTCode').csList('source', options.initData['DepositPTCode'].rows);
            getControlObject(div, 'csBPCode').csList('source', options.initData['BPCode'].rows);
            return true;
        }
        catch (err) {
    ...
    function rcdToScr(div) {
        try {
            ...
            //txtContNo,txtPenalSum,txtContName,txtCheckNo,txtDeposit
            getShowControlData(div, 'jqxInput', 'txtContNo', 'Facility', 'ContNo')
            getShowControlData(div, 'jqxInput', 'txtPenalSum', 'Facility', 'PenalSum')
            getShowControlData(div, 'jqxInput', 'txtContName', 'Facility', 'ContName')
            getShowControlData(div, 'jqxInput', 'txtCheckNo', 'Facility', 'CheckNo')
            getShowControlData(div, 'jqxInput', 'txtDeposit', 'Facility', 'Deposit')

            //getShowControlData(div, 'jqxCheckBox', 'chkContractCust', 'Facility', 'ContractCust')

            //csDepositPTCode, csBPCode
            getShowControlData(div, 'csList', 'csBPCode', 'Facility', 'BPCode')
            getShowControlData(div, 'csList', 'csDepositPTCode', 'Facility', 'PTCode')
            
            //dteContractDate
            getShowControlData(div, 'csDateTime', 'dteContractDate', 'Facility', 'ContractDate')
    ...
    function renderControl(div) {
        try {
            ...
            //建立csList
            nameArray = ["csDepositPTCode", "csBPCode"];
            oHightArray = [20, 20];
            oWidthArray = [140, 140];
            ...
            //建立Input
            nameArray = ["txtContNo", "txtContName", "txtCheckNo", "txtPenalSum", "txtDeposit"];
            oHightArray = [20, 20, 20, 20, 20];
            oWidthArray = [140, 140, 140, 140, 140];
            readerPushInput(div, nameArray, oHightArray, oWidthArray, 1)

            //建立csDateTime
            nameArray = ["dteContractDate"];
            oHightArray = [23];
            oWidthArray = [140];
            var oFormatStreem = [];
            var oDisabled = [];
