                ...
                success: function (data) {
                    try {
                        if (data.ErrorCode == 0) {
                            ...
                            $.extend(true, options.Priv, tmp.Priv);
                            delete tmp;
                            tmp = null;
   ...
   /*
            ...
            options.grdChange = {
                ...
                datafields: [
                  { name: 'KIND',type: 'string' },
                    ...
                    { name: 'FACIRECOUPSNO', type: 'integer' },
                    { name: 'PRDATE', type: 'date' },
                    { name: 'SNO', type: 'string' },
                    { name: 'FINTIME', type: 'date' },
                    { name: 'RETURNCODE',type:'integer' }
                ]
            };
           ...
           // var dataAdapter3 = new $.jqx.dataAdapter(options.grdDelete003);
            ...
            $('#' + headId + 'grdNormal').jqxGrid({
                ...
                columns: [
                    ...
                    { text:lang.SNO,datafield:'SNO',width:150}
                ],
            ...
            $('#' + headId + 'grdChange').jqxGrid({
                ...
                columns: [
                    ...
                    { text: lang.colFaciSNo2, datafield: 'FACISNO', width: 150 },
                    ...
                    { text: 'SMARTCARDNO', datafield: 'SMARTCARDNO', width: 120, hidden: true },
                    { text: 'STBSNO', datafield: 'STBSNO', width: 200, hidden: true },
                    { text: 'RESEQNO', datafield: 'RESEQNO', width: 200, hidden: true },
                    { text: 'REFNO', datafield: 'REFNO', width: 200, hidden: true },
                    { text: 'FACIRECOUPSNO', datafield: 'FACIRECOUPSNO', width: 200, hidden: true },
                    { text: 'PRDATE', datafield: 'PRDATE', width: 200, hidden: true },
                    { text: 'SNO',datafield:'SNO',width:200,hidden:false}
                ],
                localization: options.localization
            });
            //dataAdapter2 = null;
    ...
    function refreshDelGrid(div,rowIndex, act) {
        ...
        try {
            ...
            var parameters = $.extend(true, {}, loginInfo, {
                ...
                //FaciSeqno: { type: 'string', value: data2.rows[indexgrdChang].FACISEQNO },
                //FaciSeqno: { type: 'string', value: $('#' + headId + 'grdChange').jqxGrid('getcellvalue', rowIndex, "FACISEQNO") },
                FaciSeqno: { type: 'string', value: $('#' + headId + 'grdChange').jqxGrid('getcellvalue', rowIndex, "SEQNO") },
                ServiceType: { type: 'string', value: options.serviceType },
            });
            var params = getParameters(riadllName,
                                                   riaClassName,
                                    ...
                                    for (var i = 0 ; i < len ; i++) {
                                        ...
                                        for (var j = 0 ; j < len2; j++) {
                                            if (options.wipDataCopy.changeFacilityCopy.rows[i]['SEQNO'] == tmp.Table.rows[j]['SEQNO']) {
                                                ...
                                                if (tmp.Table.rows[j]['FACIRECOUPSNO'] != undefined) {
                                                    $.extend(true, options.wipDataCopy.changeFacilityCopy.rows[i], { FACIRECOUPSNO: tmp.Table.rows[j]['FACIRECOUPSNO'] });
                                                } else { $.extend(true, options.wipDataCopy.changeFacilityCopy.rows[i], { FACIRECOUPSNO: 0 }); };
                                                if (tmp.Table.rows[j]['PRDATE'] != undefined) {
                                                    $.extend(true, options.wipDataCopy.changeFacilityCopy.rows[i], { PRDATE: tmp.Table.rows[j]['PRDATE'] });
                                                } else { $.extend(true, options.wipDataCopy.changeFacilityCopy.rows[i], { PRDATE: null }); };
                                            }
    ...
    function initialGrid(div, act) {
        ...
        try {
            ...
            options.wipDataCopy.changeFacilityCopy.columns.push({ name: 'FACIRECOUPSNO', type: 'integer' });
            options.wipDataCopy.changeFacilityCopy.columns.push({ name: 'PRDATE', type: 'date' });
            var data2 = options.wipDataCopy.changeFacilityCopy;
            options.grdNormal.localdata = data1;
            options.grdChange.localdata = data2;
                ...
                success: function (data) {
                    try {
                        if (data.ErrorCode == 0) {                            
                            ...
                            source.localdata = options.wipDataCopy.canChangeKind.rows;
            ...
            getServerData(params, {
                success: function (data) {
                    try {
                        ...
                        errorHandle(formName, 'GetCPEMAC-Server', err);
    ...
    function getIPCount(type, div, act) {
        ...
        try {
            var rows = $(headerId + 'grdNormal').jqxGrid('getrows');
            if (rows.length === 0 && options.wipDataCopy.ChangeFacility.rows.length === 0) {                
                    if ($.isFunction(act)) {
                        act();
                    };                                
            } else {
                ...
                if (rows.length > 0) {
                    if (rowindex === -1) { rowindex = 0 };
                    ipCount = $(headerId + 'grdNormal').jqxGrid('getcellvalue', rowindex, fieldName);

                } else {
                    if (options.wipDataCopy.facilityTemp.rows.length > 0) {
                        ipCount = options.wipDataCopy.facilityTemp.rows[0][fieldName];
                    }                    
                };
    ...
    function getCMRateCode(type, div, act,gridEvent) {
        var options = $.data(div, formName).options;
        var headerId = '#' + $(div).prop('id');
        try {
            var rows = $(headerId + 'grdNormal' ).jqxGrid('getrows');
            if (rows.length === 0 ) {
                ...
                var rowindex = $(headerId + 'grdNormal').jqxGrid('getselectedrowindex');
                if (rowindex === -1) { rowindex = 0 };
                if (gridEvent != undefined) {
                    rowindex = gridEvent.args.rowindex;
                }
    ...
    function getDVRSizeCode(type, div, act,gridEvent ) {
        var options = $.data(div, formName).options;
        var headerId = '#' + $(div).prop('id');
        try {
            var rows = $(headerId + 'grdNormal').jqxGrid('getrows');
    ...
    function grdNormalRowSelected(event) {
        ...
        try {
            ...
            switch (item.value) {
                case 106, 107:
                    {                      
                        getDVRSizeCode(0, event.data, function () { changeMode(event.data) }, event);
                        break;
                    };
                case 107:
                    {
                        getDVRSizeCode(1, event.data, function () { changeMode(event.data) },event);
                        break;
                    }
                case 104: {
                    getCMRateCode(1, event.data, function () { changeMode(event.data) },event);
                    break;
                    }
                case 105: {
                    getCMRateCode(0, event.data, function () { changeMode(event.data) },event);
                    break;
                }
                default: {
    ...
    function isDataOK(div,act) {
        ...
        var headerId = '#' + $(div).prop('id');
        try {
            var rowSelect  = $(headerId + 'grdNormal').jqxGrid('getselectedrowindex');
            var faciKind = $(headerId + 'grdNormal').jqxGrid('getcellvalue', rowSelect, "FACIWIPSTATUS");
            if (options.wipDataCopy.Wip.rows.length > 0) {
                if ($(headerId + 'grdNormal').jqxGrid('getcellvalue', rowSelect, "SNO") != options.wipDataCopy.Wip.rows[0]['SNO']) {
                    if (faciKind == options.language.Change) {
                        messageBox(options.language.CannotUpdChanging, messageType.information);
                        changeMode(div);
                        return;
                    };
                }
                    ...
                    case 114:
                        {
                            ...
                            if ($(headerId + 'gilKind').csList('codeNo') == '' && $(headerId + 'gilKind2').csList('codeNo') == '') {
                                ...
                                if ($(headerId + 'gilKind').csList('selectedItem').IPQUANTITY == fixIPCount) {
                                    messageBox(options.language.NewIPSameOldIP, null, null,
                                               function () {
                                                   changeMode(div);
                                                   //showSO111xCEA(div);
                                                   return;
    ...
    function chkHasFinTime(div) {
        var options = $.data(div, formName).options;
        if (options.wipDataCopy.Wip.rows.length > 0) {
            if (options.wipDataCopy.Wip.rows[0].CALLOKTIME != undefined) {
                if (options.wipDataCopy.Wip.rows[0].CALLOKTIME != null && options.wipDataCopy.Wip.rows[0].CALLOKTIME != '') {
    ...
    function btnChangeFaciClick(event) {
        ...
        try {
            ...
            if ( !chkHasFinTime(event.data) ) {
                messageBox(options.language.HasFinTime, messageType.critical);
                return;
            };          
            disableAllcontrol(event.data);
            unBindHandler(event.data);
    ...
    function needToDeleteSEQNO(div, act) {
        var options = $.data(div, formName).options;
        var headerId = '#' + $(div).prop('id');
        var aryDelete = [];
        var seqNo = '';
        ...
        aryDelete.push(seqNo);
        //數位機上盒
        var lenFacility = options.wipDataCopy.Facility.rows.length;
        var lenGrd = grdRows.length;
        for (var i = 0 ; i < lenFacility; i++) {
            if (options.wipDataCopy.Facility.rows[i].RESEQNO === seqNo) {
                aryDelete.push(options.wipDataCopy.Facility.rows[i].SEQNO);
                break;
            };
        };
        if (smartCardNo !== '') {
            ...
            //ICC
            for (var i = 0; i < lenGrd; i++) {
                if ($(headerId + 'grdChange').jqxGrid('getcellvalue', i, "FACISNO") != undefined) {
                    if ($(headerId + 'grdChange').jqxGrid('getcellvalue', i, "FACISNO") === smartCardNo) {
                        aryDelete.push($(headerId + 'grdChange').jqxGrid('getcellvalue', i, "SEQNO"))
                        for (var j = 0 ; j < lenFacility; j++) {
                            if (options.wipDataCopy.Facility.rows[j].RESEQNO === $(headerId + 'grdChange').jqxGrid('getcellvalue', i, "SEQNO")) {
                                aryDelete.push(options.wipDataCopy.Facility.rows[j].SEQNO);
                                break;
                            };
                        };
                    };
    ...
    function delChangeFaciTemp(div, act) {
        ...
        if (options.showUi) {
            ...
            var aryDelete = needToDeleteSEQNO(div);
            var DVRarray = [];
            var ICCarray = [];
            var grdRows = $(headerId + 'grdChange').jqxGrid('getrows');
            if (smartCardNo != '') {
        ...
        try {
            if (rwFacilityTemp.length > 0) {                
                ...
                if (seqNo == '') {
                    ...
                    if (rwCount.length == 0) { seqNo = '' } else {
                            ...
                            options.wipDataCopy[tableName].rows = $.grep(options.wipDataCopy[tableName].rows,
                                function (row) {
                                    ...
                                    for (var i = 0; i < aryDelete.length; i++) {
                                        if (row.SEQNO === aryDelete[i]) { bln = false; };
                                    };
                                    return bln;
                                })
    ...
    function chkKind(div, Kind, rowindex) {
        var options = $.data(div, formName).options;
        var headerId = '#' + $(div).prop('id');
        if (Kind == options.language.ChangeInstall) {
            messageBox(options.language.CannotCancelChange, messageType.critical);
            return;
        };
        if (Kind == options.language.AddNew) {
            var msg = options.language.KindNotCancel;
            msg = msg.replace('{0}', options.language.AddNew);
            messageBox(msg, messageType.critical);
            return;
        };
        if (Kind == options.language.PromChange) {
            var msg = options.language.KindNotCancel;
            msg = msg.replace('{0}', options.language.PromChange);
            messageBox(msg, messageType.critical);
            return;
        };
        if (Kind == options.language.AheadCondi) {
            var msg = options.language.KindNotCancel;
            msg = msg.replace('{0}', options.language.AheadCondi);
            messageBox(msg, messageType.critical);
            return;
        }
        if (Kind == options.language.MoveInst) {
            var msg = options.language.KindNotCancel;
            msg = msg.replace('{0}', options.language.MoveInst);
            messageBox(msg, messageType.critical);
            return;
        };
        if (Kind == options.language.Retrieve) {
            if ($(headerId + 'grdChange').jqxGrid('getcellvalue', rowindex, "FACIRECOUPSNO") == 1) {
                if ($(headerId + 'grdChange').jqxGrid('getcellvalue', rowindex, "PRDATE") != null) {
                    messageBox(options.language.PRHaveFin, messageType.critical);
                    return;
                };
            } else {
                if (!getUsePriv(options.Priv, 'SO111382')) {
                    var msg = options.language.KindNotCancel;
                    msg = msg.replace('{0}', options.language.Retrieve);
                    messageBox(msg, messageType.critical);
                    return;
                };
            };
    ...
    function btnCancelChangeClick(event) {
        ...
        try {
            ...
            var rowindex = $(headerId + 'grdChange').jqxGrid('getselectedrowindex');
            var Kind = $(headerId + 'grdChange').jqxGrid('getcellvalue', rowindex, "KIND");
            if (!chkKind(event.data,Kind,rowindex)) { return; };
        ...
        try {
            ...
            $(headerId +'jqxTabs').on('selected', function (event) {
                if (event.args.item == 1 ) {
                    ...
                    if (options.createKind) {
                        $(headerId + 'gilKind').csList('resize');
                    };
                    if (options.createKind2) {
                        $(headerId + 'gilKind2').csList('resize');
                    };
    //                $(headerId + 'grdNormal').jqxGrid({ selectedrowindex: 0 });
                }
            });
