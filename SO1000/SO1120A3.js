(function ($) {
    ...
    $.fn.SO1120A3 = function (method, param) {
        try {
            ...
            return this.each(function () {
                try {
                    ...
                    if (state) {
                        ...
                        $.data(this, formName, {
                            options: $.extend({}, new defaults(), new SO1120A3(), method)
                        });
                        formLoaded(this);
                    }
                }
                catch (err) {
                    errorHandle(formName, '$.fn.SO1120A3_each', err);
                }
            });
        }
        catch (err) {
            errorHandle(formName, '$.fn.SO1120A3', err);
        }
    };
    ...
    function changeMode(div, em) {
        try {
            ...
            disableChildControls(div, getControlObject(div, 'frmSO1120A3'), options.controls, flag);
        }
        catch (err) {
    ...
    function initcsListData(div) {
        try {
            ...
            getControlObject(div, 'csCMBaudRate').csList('source', options.initData['BaudRate0'].rows);
            getControlObject(div, 'csProbationRateNo').csList('source', options.initData['BaudRate1'].rows);
            getControlObject(div, 'csVendorCode').csList('source', options.initData['VendorCode'].rows);
            getControlObject(div, 'csVendorCode2').csList('source', options.initData['VendorCode2'].rows);
            getControlObject(div, 'cboFixIPCount').csList('source', options.initData['FixIPCount'].rows);
            getControlObject(div, 'cboDynIPCount').csList('source', options.initData['DynIPCount'].rows);
            return true;
        }
        catch (err) {
    ...
    function rcdToScr(div) {
        try {
            ...
            getShowControlData(div, 'jqxInput', 'txtDialAccount', 'Facility', 'DialAccount')
            getShowControlData(div, 'jqxInput', 'txtDialPassword', 'Facility', 'DialPassword')
            getShowControlData(div, 'jqxInput', 'txtEmail', 'Facility', 'Email')
            getShowControlData(div, 'jqxInput', 'txtIPAddress', 'Facility', 'IPAddress')

            //getShowControlData(div, 'jqxCheckBox', 'chkProbationStopFlag', 'Facility', 'ProbationStopFlag')
            
            //csCMBaudRate, csProbationRateNo, csVendorCode, csVendorCode2, cboFixIPCount, cboDynIPCount
            getShowControlData(div, 'csList', 'csCMBaudRate', 'Facility', 'CMBaudNo')
            getShowControlData(div, 'csList', 'csProbationRateNo', 'Facility', 'ProbationRateNo')
            getShowControlData(div, 'csList', 'csVendorCode', 'Facility', 'VendorCode')
            getShowControlData(div, 'csList', 'csVendorCode2', 'Facility', 'VendorCode2')
            getShowControlData(div, 'csList', 'cboFixIPCount', 'Facility', 'FixIPCount')
            getShowControlData(div, 'csList', 'csProbationRateNo', 'Facility', 'DynIPCount')
           
            //dteEnableAccount,dteDisableAccount,dteProbationStopDate
            getShowControlData(div, 'csDateTime', 'dteEnableAccount', 'Facility', 'EnableAccount')
            getShowControlData(div, 'csDateTime', 'dteDisableAccount', 'Facility', 'DisableAccount')
            getShowControlData(div, 'csDateTime', 'dteProbationStopDate', 'Facility', 'ProbationStopDate')
        }
        catch (err) {
    ...
    function renderControl(div) {
        try {
            ...
            //建立csList
            nameArray = ["csCMBaudRate", "csVendorCode2", "csProbationRateNo", "csVendorCode", "cboFixIPCount", "cboDynIPCount"];
            oHightArray = [20, 20, 20, 20, 20, 20];
            oWidthArray = [150, 150, 150, 150, 150, 150];
            var oCodeNoWidth = [50, 50, 50, 50, 50, 50];
            ...
            //建立Input
            nameArray = ["txtDialAccount", "txtDialPassword", "txtEmail", "txtIPAddress"];
            oHightArray = [20, 20, 20, 20];
            oWidthArray = [150, 150, 150, 150];
            readerPushInput(div, nameArray, oHightArray, oWidthArray, 1)

            //建立csDateTime
            nameArray = ["dteEnableAccount", "dteDisableAccount", "dteProbationStopDate"];
            oHightArray = [23, 23, 23];
            oWidthArray = [150, 150, 150];
            var oFormatStreem = ["", "", ""];
            var oDisabled = ["", "", ""];
            var oShowButton = ["", "", ""];
