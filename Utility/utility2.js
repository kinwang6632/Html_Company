(function ($) {
    //$.utility = (function (method, para1, para2, para3, para4) {
    //    return eval(method + '(para1, para2, para3, para4)');
    //});
    $.utility = {
    ...
    function formatDateTime(date, datetype, mask) {
        var nd;
        if (date == null) { return; }
        if (typeof (date) == 'string') {
            if (date == null) {
        ...
        switch (datetype) {
            ...
            case 'yyyymmddhhmmssff':
                value = [year, month, day].join(maskString1) + maskSpace + [hours, minutes, seconds, milliseconds].join(maskString2);
                break;
            case "time":
            case "HHmm":
            case "hhmm":
            ...
            case "HHmmssff":
            case "hhmmssff":
                value = [hours, minutes, seconds, milliseconds].join(maskString2);
                break;
        }
        return value;
    }
    ...
    function messageBox(msg, msgType, prompt, action) {
        try {
            ...
            if (msgType == messageType.yesnocancel) {
                btnControls.push({ name: yesId, text: language.yes });
                btnControls.push({ name: noId, text: language.no });
            ...
            if (btnControls.length > 0) {
                ...
                $('<div id = "' + msgButtons + '" ></div>').appendTo('#' + msgChild);
                buttonHeight = 26;
            }
            for (var i = 0; i < btnControls.length; i++) {
                $('<input type="button" id="' + btnControls[i].name + '" style="float:left;margin:2px;width:99%;" />').appendTo($('#' + msgButtons));
