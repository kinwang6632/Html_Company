(function ($) {
    ...
    $.fn.SO1132A11 = function (options, param, param2) {
        try {
            ...
            errorHandle(formName, '$.fn.SO1132A11', err);
        }
    };
    var methods = {
        options: function (jq) {
    ...
    var defaults = (function () {
        ...
        this.deleteType = 1;        //作廢模式:0-BillNo+Item,1-MediaBillNo,2-MediaBillNo+ServiceType 
        ...
        this.mediaBillNo = null;
        this.custId = 0;
        this.isReCharge = 0;        //是否由作廢重產進入,部分條件不做檢核;0-否,1-是
        this.type = 1;              //0=BillNo+Item 1=MediaBillNo 2=MediaBillNo+ServiceType
    ...
    function checkCanXX(method, data, action) {
        try {
            ...
            if (isEmpty(data['isReCharge']) == true) {
                data['isReCharge'] = 0;
            };            

            var t = cloneDataTable(data, "Simple", -1, true, "Simple");
            ...
            var parameters = $.extend({}, cloneJSON(paraLoginInfo), {
                ...
                dsCharge: { type: 'string', value: JSON.stringify(t) },
                IsWip: { type: 'integer', value: data.wipType },
                IsReCharge: { type: 'integer', value: data.isReCharge },
                aEditMode: { type: 'integer', value: editMode.delete },
                DoProcessMark: { type: 'integer', value: 0 },
                DeleteType: { type: 'integer', value: 1 },
            });
    ...
    function checkParameters(em, data) {
        ...
        if (em != editMode.delete) { return ([false, lang.chkEditMode]); };

        //檢核欄位存不存在
        //0=BillNo+Item 1=MediaBillNo 2=MediaBillNo+ServiceType
        if (data[table].rows[0]['MediaBillNo'.toUpperCase()] == null) {
            //return ([false, 'column mediabillno not exist!!']);
            return ([false, lang.chkColumnMediaBillNo]);
        };
    ...
    function formLoaded(div) {
        try {
            ...
            if (data.Simple.rows[0]['MediaBillNo'.toUpperCase()] != null) {
                options.mediaBillNo = data.Simple.rows[0]['MediaBillNo'.toUpperCase()];
            };
    ...
    function detialAddHandler(div) {
        ...
        if ($(options.container).attr('class').indexOf('jqx-window') > 0) {
            $(options.container).on('winClosing.' + $(div).prop('id') , function (e) {
                close(div)
            });
        };
    };
                ...
                if (isOk == true) {
                    ...
                    if (options.reProduce == true) {
                        var e = $.Event("chooseOk", {
                            args: {
                                produceCharge: options.produceCharge,
                                cancelData: options.cancelData,
                                isSaved: true
                            }
                        });
                        ...
                        messageBox(lang.deleteOK, null, null, function () {
        ...
        try {
            ...
            var produceCharge = {
                ...
                ProduceCharge: {
                    ...
                    rows: [{
                        CUSTID: convertToNull(options.custId),
                        MEDIABILLNO: convertToNull(options.mediaBillNo)
                    }]
                }
            };
            ...
            var cancelData = {
                //type: 'string',
                CancelData: {
                    columns: [{ name: 'CANCELDATE', type: 'date' },
                              { name: 'CANCELCODE', type: 'string' },
                              { name: 'CANCELNAME', type: 'string' }
                    ],
                    rows: [{
                        CANCELDATE: convertToNull(getControlObject(div, 'dtCancelDate').csDateTime('getDate')),
                        CANCELCODE: convertToNull(getControlObject(div, 'csCancelCode').csList('codeNo')),
                        CANCELNAME: convertToNull(getControlObject(div, 'csCancelCode').csList('description'))
                    }]
                }
            };

            options.produceCharge = produceCharge;
            options.cancelData = cancelData;
