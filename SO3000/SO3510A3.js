            ...
            return this.each(function () {
                try {
                    ...
                    if (state) {
                        ...
                        $.data(this, formName, {
                            options: $.extend({}, new defaults(), new SO3510A3(), options)
                        });
                        formLoaded(this);
                    }
                    ...
                    errorHandle(formName, '$.fn.SO3510A3_each', err);
                }

            });
        }
        catch (err) {
            errorHandle(formName, '$.fn.SO3510A3', err);
        }
    };
        ...
        var ds = {
            SO126: {
                columns: [{ name: 'BEGINNUM', type: 'string' },
                    ...
                    { name: 'CLEARDATE', type: 'date' },
                    { name: 'NOTE', type: 'string' },
                    { name: 'NEWBEGINNUM', type: 'string' },
                    { name: 'OBEGINNUM', type: 'string' },
                    { name: 'OENDNUM', type: 'string' },
                    { name: 'NEWEMPNAME', type: 'string' },
                    { name: 'NEWEMPNO', type: 'string' },
                    { name: 'NEWGETPAPERDATE', type: 'date' },

                ],
                rows: [{
                    BEGINNUM: options.parameters[options.parentDataTableName].rows[0]['BEGINNUM'],
                    COMPCODE: options.parameters[options.parentDataTableName].rows[0]['COMPCODE'],
                    EMPNAME: options.parameters[options.parentDataTableName].rows[0]['EMPNAME'],
                    EMPNO: options.parameters[options.parentDataTableName].rows[0]['EMPNO'],
                    ENDNUM: options.parameters[options.parentDataTableName].rows[0]['ENDNUM'],
                    GETPAPERDATE: options.parameters[options.parentDataTableName].rows[0]['GETPAPERDATE'],
                    ...
                    TOTALPAPERCOUNT: options.parameters[options.parentDataTableName].rows[0]['TOTALPAPERCOUNT'],
                    RETURNDATE: options.parameters[options.parentDataTableName].rows[0]['RETURNDATE'],
                    CLEARDATE: options.parameters[options.parentDataTableName].rows[0]['CLEARDATE'],
                    NOTE: options.parameters[options.parentDataTableName].rows[0]['NOTE'],
                    NEWBEGINNUM: $(headerId + 'txtBeginNum').jqxMaskedInput('val'),
                    OBEGINNUM: options.parameters[options.parentDataTableName].rows[0]['BEGINNUM'],
                    OENDNUM: options.parameters[options.parentDataTableName].rows[0]['ENDNUM'],
                    NEWEMPNAME: $(headerId + 'csEmpName').csList('description'),
                    NEWEMPNO: $(headerId + 'csEmpName').csList('codeNo'),
                    NEWGETPAPERDATE: $(headerId + 'txtGetPaperDate').csDateTime('getDate')
